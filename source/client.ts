...
function parseSendAckResponse(value: unknown): SendAckResponse | undefined {
	...
	return value as SendAckResponse;
}
	...
	private async sendRealtimeTextItemWithRetries(
		...
		try {
			...
			if (
				...
				error.message.includes('not visible in the thread')
			) {
				this.logger.warn(
					`Instagram acknowledged a ${itemLabel} that did not persist; retrying (${
						attempt + 1
					}/3).`,
				);
				return this.sendRealtimeTextItemWithRetries(
					threadId,
					data,
					itemLabel,
					attempt + 1,
