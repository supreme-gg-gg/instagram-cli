...
export const description = 'Send a photo to a user';
...
export default function SendPhoto({args: commandArgs, options}: Properties) {
	const run = useCallback(
		async (client: InstagramClient) => {
			...
			const messageId = await client.sendPhoto(threadId, filepath);
			...
			if (options.json) {
				...
				outputText(`Photo sent to @${recipient}`);
			}
		},
